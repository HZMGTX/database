"""The one place Vault opens a SQLite connection.
...
``change_log`` rows it writes, which is what lets ``vault undo`` reverse one
...
from vault.paths import Layout
...
def sqlite_capabilities() -> dict:
    ...
    ``vault init`` calls this and refuses to create a database when something
...
class Database:
    """A handle on one Vault database file.
...
def connect_readonly(path: "str | Path", *, busy_timeout_ms: int = DEFAULT_BUSY_TIMEOUT_MS) -> sqlite3.Connection:
    ...
    Used for two things that must never be able to write: the ``vault sql``
