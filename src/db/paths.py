"""Where The database keeps things.

The database lives beside the code, in ``<repo>/data/data.db``, so that
...
def package_root() -> Path:
    """The directory holding the ``database`` package itself."""
    return Path(__file__).resolve().parent


def repo_root() -> Path:
    """The checkout that contains the ``database`` package.

    The package lives at ``<repo>/src/database`` so that the launcher script at
    ``<repo>/database`` and the package directory can share the name ``database``
...
def default_db_path() -> Path:
    """``<repo>/data/data.db`` -- the database unless told otherwise."""
    return repo_root() / "data" / "db.db"
...
class Layout:
    """Every path The database uses, derived from the database file's location.
    ...
    def ensure(self) -> "Layout":
        """Create every directory The database needs.  Idempotent."""
...
def resolve(explicit: "str | os.PathLike[str] | None" = None) -> Layout:
    ...
    # expanduser() first so that DB_PATH=~/notes.db behaves as typed.
