...
2. **A database from the future is never opened.**  If ``user_version``
   exceeds what this code knows, a newer The database has written columns this one
...
def migrate(db: Database, *, directory: "Path | None" = None, verbose: bool = False) -> List[int]:
    ...
    if highest > SCHEMA_VERSION:
        raise MigrationError(
            ...
            f"{SCHEMA_VERSION}; upgrade The database"
        )

    version_now = current_version(conn)
    if version_now > SCHEMA_VERSION:
        raise MigrationError(
            f"this database is at schema version {version_now}, but this copy of The database "
            f"only understands {SCHEMA_VERSION}.\n"
            f"A newer The database has written it. Upgrade rather than risk dropping data."
        )
