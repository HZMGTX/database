...
from db import (SCHEMA_VERSION, __version__, backup, dates, diffdb, files,
                   history, ids, model, search, sqlsh)
from db.db import Database, sqlite_capabilities
from db.migrate import MigrationError, migrate
from db.paths import DB_ENV_VAR, Layout, resolve
...
def _confirm(prompt: str, assume_yes: bool) -> bool:
    ...
    a script piping into The database must not be able to destroy data by accident.
    """
    if assume_yes:
        return True
...
def cmd_init(args) -> int:
    ...
    if missing:
        _err(f"This Python's SQLite is missing {', '.join(missing)}, which The database needs.")
    ...
    if args.json:
        ...
        _out(STYLE.dim(f"  or run `db backup` at any time."))
    db.close()
    return EXIT_OK
...
def cmd_add_file(args) -> int:
    ...
    if target.is_dir():
        _err(f"{target} is a directory. Use `db import` for a whole folder.")
        db.close()
        return EXIT_USAGE
...
def cmd_find(args) -> int:
    ...
    if args.json:
        ...
        # the output format makes `db find x --json || ...` a trap.
...
def cmd_rm(args) -> int:
    ...
    _out(STYLE.dim(f"  restore it with: db restore {ids.short(doc['uid'])}"))
    db.close()
    return EXIT_OK
...
def cmd_history(args) -> int:
    ...
    if args.json:
        ...
        if entries:
            _out()
            _out(STYLE.dim(f"  db diff {args.ref} {entries[-1].rev} {entries[0].rev}"))
            _out(STYLE.dim(f"  db revert {args.ref} <rev>"))
    db.close()
    return EXIT_OK
...
def cmd_connect(args) -> int:
    from db.connectors import FORMATS as CONNECTORS
    from db.connectors import vyrex as vyrex_connector
...
def cmd_export(args) -> int:
    ...
    if fmt in directory_formats or (fmt == "csv" and args.out and
        ...
        with open(args.out, "w", encoding="utf-8", newline="") as handle:
            count = handler(db, handle, query=query, include_trashed=args.trashed,
                            title=args.title or "The database")
        target = str(args.out)
    else:
    ...
    if args.json:
        ...
        if fmt == "jsonl":
            _out(STYLE.dim("  this is the lossless format; `db import` reads it back"))
        elif fmt == "html":
    ...
    if args.json:
        ...
        if result.dry_run:
            ...
            _out(STYLE.dim(f"  undo all of this with: "
                           f"db import undo {result.batch_uid[:12]}"))
    db.close()
    return EXIT_OK


def cmd_import_list(args) -> int:
    from db.importers import batch as batch_mod
    db = _open_db(args)
...
def cmd_import_undo(args) -> int:
    ...
    _emit_json(result) if args.json else _out(
        ...
        f"{STYLE.dim('  they are recoverable; `db trash --empty` removes them for good')}")
    db.close()
    return EXIT_OK
...
def cmd_backup(args) -> int:
    ...
    if args.bundle:
        from pathlib import Path as _Path
        target = _Path(args.out or (db.layout.root / "database-bundle.zip"))
...
def cmd_backups(args) -> int:
    ...
    if args.json:
        ...
        _out(STYLE.dim("No backups yet. Run `db backup`."))
    else:
...
def cmd_serve(args) -> int:
    ...
    url = server.base_url
    _out(f"{STYLE.bold('The database')} is serving at {STYLE.cyan(url)}")
    if server.lan:
...
def cmd_install_cli(args) -> int:
    """Put `database` on PATH by symlinking the launcher.
    ...
    launcher = _Path(__file__).resolve().parent.parent.parent / "database"
    if not launcher.is_file():
        _err(f"cannot find the launcher at {launcher}")
        return EXIT_NOT_FOUND

    target_dir = _Path(args.to).expanduser()
    target = target_dir / "database"
...
def _report_path(target_dir, args) -> int:
    ...
    if on_path:
        _out(STYLE.dim("  that directory is on your PATH; `database` works from anywhere now"))
    else:
        _out(STYLE.yellow(f"  {target_dir} is NOT on your PATH, so `database` will not be found."))
...
BASH_COMPLETION = """# db completion for bash
# add to ~/.bashrc:   source <(db completion bash)
_db_complete() {
...
ZSH_COMPLETION = """# db completion for zsh
# add to ~/.zshrc:   source <(db completion zsh)
_db() {
  if (( CURRENT == 2 )); then
    compadd %(names)s
  else
...
FISH_COMPLETION = """# db completion for fish
# save as ~/.config/fish/completions/db.fish
complete -c db -f
for c in %(names)s
  complete -c db -n "__fish_use_subcommand" -a $c
end
"""
...
def cmd_demo(args) -> int:
    ...
    if args.json:
        ...
        _out(STYLE.dim("  db find budget"))
        _out(STYLE.dim("  db find kind:task status:todo"))
        _out(STYLE.dim("  db today"))
    db.close()
    return EXIT_OK
...
def _add_common(parser: argparse.ArgumentParser, *, top_level: bool = False) -> None:
    ...
    parser.add_argument("--db", metavar="PATH",
                        ...
                        help=f"database file (default: <repo>/data/data.db, or ${DB_ENV_VAR})")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        epilog="Run `db demo` to load sample data, then `db find budget`.",
        formatter_class=argparse.RawDescriptionHelpFormatter)
    parser.add_argument("--version", action="version",
                        version=f"database {__version__} (schema v{SCHEMA_VERSION})")
    ...
    p = sub.add_parser("find", help="search everything",
                       description="Examples:\n"
                                   ...
                                   "shell treats them as redirects and The database never sees them.",
                       formatter_class=argparse.RawDescriptionHelpFormatter)
    ...
    sp = sub.add_parser("import-list", help="past imports (also: db import list)")
    sp.add_argument("--limit", type=int, default=20)
    _add_common(sp); sp.set_defaults(func=cmd_import_list)

    sp = sub.add_parser("import-undo", help="reverse an import (also: db import undo)")
    ...
    p = sub.add_parser("install-cli", help="put `database` on your PATH")
...
def _overview(args) -> int:
    ...
    if not layout.db.exists():
        _out(STYLE.bold("The database") + STYLE.dim(f"  {__version__}"))
        _out()
        _out("  No database yet. Create one with:")
        _out(STYLE.cyan("    db init"))
        _out(STYLE.dim("    db demo      # and load some sample data to explore"))
        return EXIT_OK
    ...
    db.close()
    _out(STYLE.bold("The database") + STYLE.dim(f"   {count:,} items   {layout.db}"))
    _out()
    cmd_today(args)
    _out(STYLE.dim("  db find <query>     db add note \"...\"     db --help"))
    return EXIT_OK
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    try:
        ...
        # `db find x | head` is a normal thing to do.
        try:
            sys.stdout.close()
        except Exception:
            pass
