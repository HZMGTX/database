...
write-ahead log. ``db backup`` is the version that is always safe, so it
...
STAMP = re.compile(r"database-(\d{8}T\d{6}Z)")
...
def take(db: Database, *, kind: str = "manual", target: Optional[Path] = None,
    ...
    path = Path(target) if target else layout.backups / f"database-{_stamp()}.db"
    if path.exists():
        path.unlink()
...
def prune(db: Database, *, keep_daily: int = 7, keep_weekly: int = 4,
    ...
    for path in layout.backups.glob("database-*.db"):
    ...
    try:
        with zipfile.ZipFile(target, "w", zipfile.ZIP_DEFLATED, compresslevel=6) as archive:
            ...
            archive.writestr("RESTORE.txt",
                             "This bundle holds a complete The database.\n\n"
                             "  data.db   the database\n"
                             "  files/     attachments, laid out as The database expects\n\n"
                             "To restore: unzip it, put data.db and files/ in your data\n"
                             "directory, and run `db doctor` to confirm.\n")
    finally:
