...
``db.secretscan`` for why that matters here specifically.
...
from db.importers.directory import DEFAULT_MAX_BYTES, should_skip
...
def _metadata_only(db: Database, batch: Batch, path: Path, relative: Path,
    ...
    Deliberately not an attachment: ``db add file`` stores the bytes
