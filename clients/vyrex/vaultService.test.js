...
/**
 * The Vault bridge has one job beyond forwarding: it must never be the reason
 * a command handler throws. A Vault that is not running is the ordinary case,
 ...
 *
 * A stub HTTP server stands in for Vault, which keeps the test hermetic and
...
before(async () => {
  ...
  process.env.VAULT_URL = `http://127.0.0.1:${server.address().port}`;
  delete process.env.VAULT_TOKEN;
  delete process.env.VAULT_DEBUG;
  vault = require('../src/services/vaultService');
});
...
test('search sends the query and returns the hits', async () => {
  ...
  const hits = await vault.search('kind:task status:todo', { limit: 5 });
...
test('search returns an empty array when Vault answers with an error', async () => {
  handler = json(400, { title: 'Bad query', detail: 'unknown flag is:nonsense' });

  assert.deepEqual(await vault.search('is:nonsense'), []);
});
...
test('search rethrows the problem detail in strict mode', async () => {
  ...
  await assert.rejects(
    () => vault.search('is:nonsense', { strict: true }),
    (error) => {
...
test('two captures use different idempotency keys', async () => {
  ...
  await vault.capture({ title: 'one' });
  await vault.capture({ title: 'two' });
...
test('capture refuses an item with no title, without calling out', async () => {
  assert.equal(await vault.capture({ body: 'orphaned text' }), null);
...
test('a slow server times out rather than hanging a handler', async () => {
  ...
  const hits = await vault.search('slow', { limit: 1 });
...
test('a server that is not running yields empty results, not a throw', async () => {
  // The ordinary case: Vault is a separate process somebody has to start,
  // so it is down more often than it is up.
  const good = process.env.VAULT_URL;
  process.env.VAULT_URL = 'http://127.0.0.1:9';
  try {
    assert.deepEqual(await vault.search('nobody home'), []);
    assert.equal(await vault.capture({ title: 'nobody home' }), null);
    assert.equal(await vault.get('abcdef12'), null);
    assert.equal(await vault.available(), false);
  } finally {
    process.env.VAULT_URL = good;
  }

  // ...and it recovers the moment the server is back.
  assert.deepEqual(await vault.search('still fine'), []);
  assert.equal(received.length, 1);
});

test('VAULT_URL is honoured when it changes, and trailing slashes are trimmed', async () => {
  const good = process.env.VAULT_URL;
  process.env.VAULT_URL = `${good}///`;
  try {
    await vault.search('trimmed');
...
test('the bearer token is sent only when one is configured', async () => {
  ...
  await vault.search('no token');
  assert.equal(received[0].headers.authorization, undefined);

  process.env.VAULT_TOKEN = 'secret-token';
  try {
    await vault.search('with token');
...
test('get looks an item up by its short handle', async () => {
  ...
  const item = await vault.get('19b3cd5e');
...
test('renderSnippet replaces the highlight markers', () => {
  const snippet = `Please review the ${vault.MARK_START}budget${vault.MARK_END} before Friday.`;

  assert.equal(vault.renderSnippet(snippet),
               'Please review the budget before Friday.');
  assert.equal(vault.renderSnippet(snippet, { start: '**', end: '**' }),
               'Please review the **budget** before Friday.');
  ...
  assert.equal(vault.renderSnippet(''), '');
  assert.equal(vault.renderSnippet(undefined), '');
});
